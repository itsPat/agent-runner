...
A distributed AI agent runner. You give it a goal in natural language; an AI decomposes it into a DAG of subtasks; a Go orchestrator executes the DAG across a worker pool with retries, timeouts, and live progress streaming to a React frontend.
...
2. The Go orchestrator forwards the goal to the AI Planner Service (TypeScript + ai-sdk).
3. The AI Planner returns a structured task DAG over gRPC (nodes = tasks, edges = dependencies).
4. The orchestrator persists the DAG to CockroachDB and begins execution.
...
|---|---|---|
| **Orchestrator** | Go | DAG execution, worker pool, state management, client API, SSE streaming |
| **AI Planner/Executor** | TypeScript (ai-sdk, bun runtime) | Goal decomposition, AI task execution, structured output |
...
- **Frontend ↔ Orchestrator:** HTTP for commands, SSE for live DAG updates.
- **Orchestrator ↔ AI Service:** gRPC with protobuf. This is where the protobuf contract lives.
- **Orchestrator ↔ Cockroach:** standard SQL via `pgx`.

## Why each technology

- **Go orchestrator.** The heart of the project. Worker pool, context cancellation, channel-based coordination, graceful shutdown, retries with backoff — textbook Go.
...
- **Protobuf / gRPC.** The orchestrator ↔ AI service contract is well-typed and versionable. Task definitions, results, and DAG structures are all structured data — a perfect fit.
...
│   ├── go.mod
│   └── Dockerfile
├── ai-service/         # TS service (bun runtime)
│   ├── src/
│   ├── package.json
│   └── Dockerfile
...
│   ├── src/
│   └── package.json
├── docker-compose.yml  # cockroach + orchestrator + ai-service
...
Streaming responses let the frontend see AI tokens arrive live — Go forwards the gRPC stream into the SSE connection.
...
# Bring up cockroach, orchestrator, ai-service
make up
...
| `make up` | Start cockroach + orchestrator + ai-service |
...
- Single-node orchestrator. No multi-node coordination.
...
- Persistent task queues across orchestrator restarts (stretch goal)